...
from torch.distributed.elastic.control_plane import main, TORCH_WORKER_SERVER_SOCKET
...
def local_worker_server() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with main():
            ...
            session.mount("http+unix://", UnixHTTPAdapter(socket_path))
            yield session
...
class WorkerServerTest(TestCase):
    def test_worker_server(self) -> None:
        with local_worker_server() as session:
            resp = session.get("http+unix://worker/")
...
""",
            ...
            resp = session.post("http+unix://worker/handler/ping")
            self.assertEqual(resp.status_code, 200)
            self.assertEqual(resp.text, "pong")

            resp = session.get("http+unix://worker/handler/")
            self.assertEqual(resp.status_code, 200)
